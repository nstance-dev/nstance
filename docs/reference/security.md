...
On Google Cloud, `google-secret-manager` is the default direct secrets store. The Nstance server receives only the Secret Manager permissions needed to create, read, update, list, and delete secrets. Encrypted `object-storage` is available as an explicit alternative.
