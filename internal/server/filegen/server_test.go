...
// TestGenerateServerFilesUsesSharedFileSources verifies server files use normal file sources.
func TestGenerateServerFilesUsesSharedFileSources(t *testing.T) {
	...
	cfg := &config.Config{
		...
		Defaults: config.DefaultsConfig{Vars: map[string]string{"Name": "server"}},
		Server: config.ServerRuntimeConfig{Files: map[string]config.FileConfig{
	...
	files, err := generator.GenerateServerFiles(ctx, cfg)
	if err != nil {
		t.Fatal(err)
	}
